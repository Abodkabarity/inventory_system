  ...
  class OrdersBloc extends Bloc<OrdersEvent, OrdersState> {
    ...
    Future<void> _onLoadAll(OrdersLoadAll e, Emitter<OrdersState> emit) async {
      ...
      try {
        ...
        final branchInfo =
        await repo.fetchBranchInfo(
          branchName: state.branchName,
        );
  
        final submitStartHour =
            branchInfo['submit_start_hour'] ?? 21;
  
        final submitEndHour =
            branchInfo['submit_end_hour'] ?? 9;
        final maxAdjLimit =
            branchInfo['max_adj_limit'] ?? 25;
  
        final orderIncreaseLimit =
            branchInfo['order_increase_limit'] ?? 10;
  
        final orderEditLimit =
            branchInfo['order_edit_limit'] ?? 20;
  
        final additionalOrderLimit =
            branchInfo['additional_order_limit'] ?? 10;
        final usedSlots =
            branchInfo['used_slots'] ?? 0;
  
        final remainingSlots =
            branchInfo['remaining_slots'] ?? 0;
        final nextAvailableDate =
        branchInfo['next_available_date'];

        final daysUntilNextSlot =
        branchInfo['days_until_next_slot'];
        emit(
          state.copyWith(
            status: OrdersStatus.loading,
            progress: 93,
    ...
    Future<void> _onLoadMaxAdj(
      ...
      final data = await repo.fetchMaxAdj(
        branch: state.branchName,
      );
