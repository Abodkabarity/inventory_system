...
class OrdersBloc extends Bloc<OrdersEvent, OrdersState> {
  ...
  Future<void> _onLoadAll(OrdersLoadAll e, Emitter<OrdersState> emit) async {
    ...
    try {
      ...
      final usedAdditionalOrders = trackingRaw.length;
      ...
      final submitStartHour = branchInfo['submit_start_hour'] ?? 21;

      final submitEndHour = branchInfo['submit_end_hour'] ?? 9;
      final maxAdjLimit = branchInfo['max_adj_limit'] ?? 25;

      final orderIncreaseLimit = branchInfo['order_increase_limit'] ?? 10;

      final orderEditLimit = branchInfo['order_edit_limit'] ?? 20;

      final additionalOrderLimit = branchInfo['additional_order_limit'] ?? 10;
      final usedSlots = branchInfo['used_slots'] ?? 0;

      final remainingSlots = branchInfo['remaining_slots'] ?? 0;
      final nextAvailableDate = branchInfo['next_available_date'];

      final daysUntilNextSlot = branchInfo['days_until_next_slot'];
      emit(
        state.copyWith(
          status: OrdersStatus.loading,
          progress: 93,
          ...
          maxAdjLimit: maxAdjLimit,
          orderIncreaseLimit: orderIncreaseLimit,
          orderEditLimit: orderEditLimit,
          usedMaxAdjSlots: usedSlots,
          usedAdditionalOrders: usedAdditionalOrders,
          nextAvailableDate: nextAvailableDate?.toString(),

          daysUntilNextSlot: daysUntilNextSlot is num
              ? daysUntilNextSlot.toInt()
              : null,
          remainingMaxAdjSlots: remainingSlots,
  ...
  Future<void> _onApplyAdditionalRequest(
    ...
    emit(
      state.copyWith(
        ...
        usedAdditionalOrders: state.additionalTrackingRows.length + next.length,
      ),
    );
  }
  ...
  Future<void> _onSendAdditionalRequestsPressed(
    ...
    try {
      ...
      final trackingRows = trackingRaw
          .map(AdditionalRequestRow.fromMap)
          .toList();
      final usedAdditionalOrders = trackingRows.length;
      final view = _applyUiFilters(
        rows: _applySearch(state.rows, state.search),
        categoryFilter: state.categoryFilter,
        formularyFilter: state.formularyFilter,
  ...
  Future<void> _onLoadMaxAdj(
    ...
    final branchInfo = await repo.fetchBranchInfo(branchName: state.branchName);

    emit(
      state.copyWith(
        status: OrdersStatus.ready,
        maxAdjItems: data,
        usedMaxAdjSlots: branchInfo['used_slots'] ?? 0,
        remainingMaxAdjSlots: branchInfo['remaining_slots'] ?? 0,
  ...
  Future<void> _onDeleteItemToOrder(
    ...
    final itemsRaw = await repo.fetchItemsToOrder(
      runDate: state.runDate,
      branchName: state.branchName,
    );

    final items = itemsRaw.map(ItemToOrder.fromMap).toList();
