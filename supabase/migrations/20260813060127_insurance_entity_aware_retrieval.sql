create or replace function public.search_insurance_knowledge_v2(
  query_text text,
  ...
  with params as (
    ...
    select
      ...
      case
        ...
        when nullif(c.metadata->>'entity_name_normalized', '') is null then 'accepted_untagged_context'
    ...
    select r.*,
      (
        (r.lexical_score * 0.55)
        + (case when query_embedding is null then 0 else r.semantic_score * 0.25 end)
        + (case when r.entity_score = 1 then 0.75 when r.entity_score = -1 then -0.90 else 0 end)
...
revoke all on function public.search_insurance_knowledge_v2(
  text, extensions.vector, integer, boolean, uuid, uuid
) from public, anon;
grant execute on function public.search_insurance_knowledge_v2(
  text, extensions.vector, integer, boolean, uuid, uuid
